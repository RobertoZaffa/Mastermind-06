...
label start:
    $ quick_menu = False
    $ game_loop()
    $ renpy.quit() #comment this row if Mastermind is launched from another script 
...
init 1 python:
            ...
            while inserting_code:
                ...
                if persistent.status == "final":     #valore impostato dalla funzione di Input. Click sullo shield
                    ...
                    if yes_no == "yes":
                        ...
                        persistent.choice = "game1_" + str(colors)
                        persistent.status = "on_game"
    ...
    def game4(colors):
        ...
        renpy.hide_screen ("open_shield")
        renpy.hide_screen("board_update") 
        #narrator ("prima di enter")
        #renpy.call_screen ("confirm_game4", "Which version of the game do you want to play?")
        renpy.call_screen ("secret_code_game4", "In GAME-2 the App will guess your secret code.\n\n Do you want to enter the secret code or keep it in mind?")
        if secret == "Enter":
            #narrator ("inizio ciclo")
            renpy.show_screen("open_shield")
            while not clicked == 10:       
                draggable = False
                droppable = False
                renpy.show_screen("board_update")
                renpy.call_screen ("enter_secret_code_game4", colors, secret_code)
                for peg in range (1, colors+1):     # se è stato droppato un peg della colonna esterna, viene aggiornato code[]
                    for hole in range (1,5):        
                        if draggable == "peg_" + str(peg) and droppable == "hole_" + str(hole):
    ...
    def mm_final (blacks, row):            # prepara il finale del gioco e chiama board_update
        ...
        if (blacks == 4 or row == 10) and (persistent.choice == "game1_6" or persistent.choice == "game1_8"):
        ...
        if blacks == 4 and persistent.choice == "game4":
...
screen board_update():      # aggiorna i pegs sulla board e se chiamato da mm_final visualizza il codice segreto
    fixed:
        xsize 1080
        ysize 1920
...
screen score_update():      # aggiorna i pegs sulla board e se chiamato da mm_final visualizza il codice segreto
    fixed:
        xsize 1080
        ysize 1920
        for i in range (1,30):              #aggiorna la board con le giocate
            for j in range (1,3):
...
screen mm_menu():   # è il menu contestualizzato al richiamo delle modalità di gioco oppure mentre si gioca la voce per arrendersi
    ...
    frame:
        ...
        vbox:            
            ...
            if persistent.choice == "none":        
                textbutton "• PLAY GAME 1":
                    action Hide("mm_menu"), SetVariable("persistent.choice", "game1_6"), Return()   
                # textbutton "• Game - 8 Colors":
                #     action Hide("mm_menu"), SetVariable("persistent.choice", "game1_8"), Return()
                textbutton "• PLAY GAME 2":
                    action Hide("mm_menu"), SetVariable("persistent.choice", "game4"), Return()
                ...
                #         action Hide("mm_menu"), SetVariable("persistent.best_code_active", False), Show("disable_best_code"), Return()
                # if not persistent.best_code_active:
                #     textbutton "• ENABLE BEST CODE":
                #          action Hide("mm_menu"), SetVariable("persistent.best_code_active", True), Show("disable_best_code"), Return()
            if persistent.choice == "game1_6" or persistent.choice == "game1_8":
                if persistent.best_code_active:
                    if not thread.is_alive():       #i best code sono stati generati
                        textbutton "• PLAY THE BEST\n CODE FOR ME":
                            action Hide("mm_menu"), SetVariable("persistent.status", "best_code"), SetVariable("best_code_ready", True), Return()
                    else:
                        text "• SERCHING BEST CODE\n      ...PLEASE WAIT...":
                            size 45
                            #color "#8c8c8c"
                            # align (0.0)
                            #bold True          
            if persistent.choice == "game1_6" or persistent.choice == "game1_8" or persistent.choice == "game4":
                textbutton "• CANCEL":
...
screen input_game1(row, colors, code):     # input delle giocate + checkmark verde + bottone di rinuncia
    fixed:
        xsize 1080
        ysize 1920
        draggroup:
            ...
            for j in range (1, 5):      # per la rimozionea dei pegs already dropped per la riga in progress
                ...
                if code[j-1] > 0:
                    drag:
                        ...
                        drag_raise False
                        mouse_drop True
            if row > 1:                 # per il drag dei peg della riga precedente all'attuale
...
screen input_game4(row, key_code):     # input delle giocate + checkmark verde + bottone di rinuncia
    fixed:
        xsize 1080
        ysize 1920
        draggroup:
            ...
            for j in range (1, 5):      # riga di drop vuoti per posizionare i key pegs
                if key_code[j-1] == 0:
                    ...
                    drag:
                        ...
                        droppable True
                        mouse_drop True  
            for j in range (1, 5):      # per la rimozionea dei key pegs already dropped per la riga in progress
...
screen secret_code_game4(message):
...
screen attempt_info(chance, tot_lc, left_code, left_code_bs, code_status, bs_inconsistent):
    ## Ensure other screens do not get input while this screen is displayed.
    modal True
    zorder 200
    style_prefix "confirm"
...
screen enter_secret_code_game4(colors, secret_code):     # input del codice segreto per Game4
    fixed:
        xsize 1080
        ysize 1920
        draggroup:
