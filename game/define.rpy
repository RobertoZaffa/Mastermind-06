...
image key_empty:
    "03_key_empty.png"
    #"03_key_empty_test.png"
    zoom 0.12

image key_empty_circle:
    #"azzurro_1600"
    "00_empty_circle_1600.png"
    zoom 0.08
...
define v2 = -146
define pegs_vert = [0, 0+7, v2+3, v2*2, v2*3, v2*4, v2*5+5, v2*6, v2*7+8, v2*8+8, v2*9+9, v2*10-35] #distanza verticale tra le giocate di input. In teoria dovevano essere gli stessi valori della board. define 
define s1 = 123
...
define key_vert_sopra =  [0, v4, v3+v4, v3*2+v4-5, v3*3+v4-5, v3*4+v4-5, v3*5+2+v4-5, v3*6+4+v4, v3*7+5+v4, v3*8+5+v4, v3*9+13+v4, v3*10+8+v4] #distanza verticale tra le giocate di tutta la board 
define key_vert_sotto =  [0, 0, v3, v3*2-5, v3*3-5, v3*4-5, v3*5+2-5, v3*6+4, v3*7+5, v3*8+5, v3*9+10, v3*10+5] #distanza verticale tra le giocate di tutta la board 
...
init 0 python:
    ...
    for i in range (1,12):
        for j in range (1,5):
            ...
            peg_pos[i][j][2] = 1807 + pegs_vert[i]          # posizione verticale
    ...
    for i in range (1,11):
        for j in range (1,5):
            if j == 1 or j ==3:                                     #se i key peg sono sulla sx
                key_pos[i][j][1] =  191 + key_oriz[1]               # posizione orizontale
            if j == 2 or j ==4:                                     #se i key peg sono sulla dx
                key_pos[i][j][1] =  191 + key_oriz[2]               # posizione orizontale
    ...
    for i in range (1,30):
        for j in range (1,3):
            score_pos[i][j][1] =  726 + score_oriz[i]*j        # posizione orizontale
            score_pos[i][j][2] = 1817 + score_vert[i]          # posizione verticale
...
default persistent.show_score_update = "not_show"   # "game1-game2" "game3"
